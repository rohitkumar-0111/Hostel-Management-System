  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-white">Student Complaints</h2>
          <p className="text-gray-500 text-sm">Manage and resolve hostel maintenance issues</p>
        </div>
        <div className="flex items-center gap-2 bg-white/5 p-1 rounded-xl border border-white/10">
          {["All", "Submitted", "InProgress", "Resolved"].map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${filter === f ? "bg-blue-500 text-white shadow-lg" : "text-gray-400 hover:text-white"
                }`}
            >
              {f}
            </button>
      ...
      <div className="grid grid-cols-1 gap-4">
        {loading ? (
          <div className="py-20 text-center text-gray-500">Loading complaints...</div>
              ...
              <div className="flex flex-row lg:flex-col justify-end gap-2">
                {complaint.status === 'Submitted' && (
                  <button
                    ...
                    className="px-4 py-2 rounded-xl bg-blue-500/10 text-blue-400 text-xs font-bold hover:bg-blue-500 text-white transition-all"
                  >
                    Mark In Progress
                  </button>
                )}
                {complaint.status !== 'Resolved' && (
                  <button
                    ...
                    className="px-4 py-2 rounded-xl bg-emerald-500/10 text-emerald-500 text-xs font-bold hover:bg-emerald-500 text-white transition-all"
                  >
                    Resolve
                  </button>
                )}
                <button className="px-4 py-2 rounded-xl bg-white/5 text-gray-400 text-xs font-bold hover:text-white transition-all">
                  View Details
                </button>
              </div>
            </div>
